...
const Cancel = ({cancel}) => {
  // Sort bookings by date in descending order (newest first)
   const sortedComplete = [...cancel].sort((a, b) => new Date(b.date) - new Date(a.date));
   ...
   return (
     <div className="w-full max-w-7xl mx-auto px-4 md:px-8 space-y-4">
       {sortedComplete.length === 0 ? (
         <p className="text-center text-gray-600">No Canceled Bookings</p>
       ) : (
           ...
           {currentItems.map((booking) => {
             ...
             return (
                    <Link
                      ...
                      className='m-2'
                      >
               <div key={booking._id} className="bg-white shadow-md rounded-lg p-4 flex justify-between items-center cursor-pointer hover:shadow-2xl">
