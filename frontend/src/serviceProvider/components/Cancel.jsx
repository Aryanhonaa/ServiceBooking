...
const Cancel = ({cancel}) => {
    const sortCancel=[...cancel].sort((a,b)=> new Date(a.date)- new Date(b.date));


    const [currentPage, setCurrentPage]=useState(1);
    const itemsPerPage=5;

    const totalPages= Math.ceil(sortCancel.length/ itemsPerPage);
    const indexOfLastItem=currentPage * itemsPerPage;
    const indexofFirstItem= indexOfLastItem- itemsPerPage;
    const currentItem= sortCancel.slice(indexofFirstItem, indexOfLastItem);
  return (
    <div>
     {
        sortCancel.length ===0 ?
        <p>No Cancel Appointments</p>:
        (
            <>
            {
                currentItem.map((booking)=>{
