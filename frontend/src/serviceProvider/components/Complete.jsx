...
const Complete = ({complete}) => {
  ...
  const sortComplete= [...complete].sort((a,b)=>new Date(b.date)- new Date(a.date));
// 
  const [currentPage, setCurrentPage]=useState(1);
  const itemsPerPage=5;

  const totalPages= Math.ceil(sortComplete.length/ itemsPerPage);
  const indexOfLastItem= currentPage * itemsPerPage;
  const indexofFirstItem= indexOfLastItem - itemsPerPage;
  const currentItems = sortComplete.slice(indexofFirstItem, indexOfLastItem);
  return (
    <div>
   {
    complete.length ===0 ?(
      <>
      <h1>No Completed Appointments</h1>
      </>
    ):(
      <>
